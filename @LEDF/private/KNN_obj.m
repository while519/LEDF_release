    ...
    switch lower(obj_para{ii})
        ...
        case 'knn'
            if isscalar(obj_para{ii+1})
                ...
                knn_col = obj_para{ii+2};
            else
                knn_row = obj_para{ii+1}{1,1};
                knn_col = obj_para{ii+1}{1,2};
            end
        otherwise
...
switch lower(obj_option)
    ...
    case 'logic_knn'
        ...
        elseif ~isnan(knn_row) && ~isnan(knn_col)
            ...
            for ii = 1:M
                R1(idx(1:row_knn(ii),ii)+N*(ii-1)) = 1;
            end
            R1 = sparse(R1') ;
            ...
            column_knn = sum(Rq,1);
            K = max(column_knn);
            [~, idx] = pdist2(X, Y, 'euclidean', 'Smallest', K);
            for ii = 1 : N
                R2(idx(1:column_knn(ii), ii) + M*(ii-1)) = 1;
            end
            Rqz = R1.*sparse(R2).*Rq;
        end
